...
void debug_to_uart(char *fmt,...) {
	...
	vsnprintf(buffer/*+2*/, 255, fmt, ParamList);
	va_end(ParamList);

	SERIAL_ECHOLN(buffer);

	/*while(*ptr){
		while(MSERIAL(MKS_WIFI_SERIAL_NUM).availableForWrite() == 0) {
			TERN_(USE_WATCHDOG, hal.watchdog_refresh());
			safe_delay(10);				
		}
		MSERIAL(MKS_WIFI_SERIAL_NUM).write(*ptr++);
	}*/
}
#endif

void mks_wifi_init() {
    SERIAL_ECHO_MSG("Init MKS WiFi module");
    MKS_WIFI_DEBUG("Init MKS WiFi module");
...
// Получает данные из всех функций, как только есть перевод строки 0x0A, формирует пакет для ESP и отправляет
	...
	switch(mks_in_packer_status) {
		...
		case ESP_IN_PACKER_DATALEN2:
			...
			if(mks_in_frame.dataLen > ESP_PACKET_DATA_MAX_SIZE) {
				MKS_WIFI_DEBUG("[ERROR] Invalid packet length from ESP: %d", mks_in_frame.dataLen);
		...
		case ESP_IN_PACKER_TAIL:
			...
			if(data != ESP_PROTOC_TAIL) {
				MKS_WIFI_DEBUG("[ERROR] Invalid tail byte from ESP");
				mks_in_packer_status = ESP_IN_PACKER_IDLE;
			} else {
	...
	switch(packet->type) {
		...
		case ESP_TYPE_GCODE: {
			for(uint16_t i = 0; i < packet->dataLen; i++) {
				//Иногда команды разделяются \n, иногда \0, возможно и \r
				if(packet->data[i] != '\n' && packet->data[i] != '\r' && packet->data[i] != '\0' && mks_in_gcode_cmd_idx < 127) {
		...
		case ESP_TYPE_FILE_FIRST: {
			MKS_WIFI_DEBUG("[FILE_FIRST]");
			//Передача файла останавливает все процессы, поэтому печать в этот момент не возможна.
			if (!CardReader::isPrinting()) {
				mks_wifi_start_file_upload(packet);
			} else {
				//Попытка запустить передачу файла во время печати. Отключение модуля wifi (зачем, бл?)
				MKS_WIFI_DEBUG("Shutdown wifi");
				WRITE(MKS_WIFI_IO_RST, LOW);
				delay(50);
				WRITE(MKS_WIFI_IO_RST, HIGH);
			}
		...
		case ESP_TYPE_FILE_FRAGMENT: {
			MKS_WIFI_DEBUG("[FILE_FRAGMENT]");
		}
		break;
		case ESP_TYPE_WIFI_LIST: {
			MKS_WIFI_DEBUG("[WIFI_LIST]");
...
void mks_wifi_update_netstatus() {
	...
	if(mks_global_net_status.state == ESP_NET_WIFI_CONNECTED) {
		if(show_ip_once == 0) {
			...
			MKS_WIFI_DEBUG("[NET] connected, %s", str);
		}
	} else if(mks_global_net_status.state == ESP_NET_WIFI_EXCEPTION) {
		MKS_WIFI_DEBUG("[NET] WiFi exeption");
	} else {
		MKS_WIFI_DEBUG("[NET] WiFi not config");
	}
}
...
void mks_wifi_send(uint8_t *packet, uint16_t size) {
	...
	if(error_flag == 1){
		if(MSERIAL(MKS_WIFI_SERIAL_NUM).availableForWrite() == 0) {
			return;
		} else {
			error_flag = 0;
		}
	...
	for(uint32_t i = 0; i < (uint32_t)(size + 1); i++) {
		...
		MSERIAL(MKS_WIFI_SERIAL_NUM).write(packet[i]);
	}
}
...
uint16_t mks_wifi_build_packet(uint8_t *packet, ESP_PROTOC_FRAME *esp_frame) {
	...
	for(uint32_t i = 0; i < esp_frame->dataLen; i++) {
		//4 байта заголовка отступить
		packet[i + 4] = esp_frame->data[i];
	}

	packet_size = esp_frame->dataLen + 4;

	if(packet_size > MKS_TOTAL_PACKET_SIZE) {
		MKS_WIFI_DEBUG("[ERROR] ESP packet too big");
		return 0;
	}
...
// проверяет, что символы из текстового диапазона для G-code команд
